...
async function getActivitiesForWbs({ wbsId, proposalId }: { wbsId: string; proposalId: string; }): Promise<Activity[]> {
    const activitiesRef = collection(firestore, "activities");
    const q = query(activitiesRef, where("wbsId", "==", wbsId));
...
function calculateWbsTotals(activities: Activity[]): { [key: string]: number } {
