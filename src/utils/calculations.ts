...
const sumRates = (...rates: number[]): number => rates.reduce((acc, rate) => acc + pctToDecimal(rate), 0);
...
export const getCraftLoadedRate = ({
    ...
    const { craftBaseRate, burdenRate, overheadRate, laborProfitRate, fuelRate, consumablesRate, subsistenceRate } = proposal;
    const craftBase = customCraftBaseRate || craftBaseRate;
    const subsistence = customSubsistenceRate || subsistenceRate;
    const totalRateSum = sumRates(burdenRate!, overheadRate!, laborProfitRate!, fuelRate!, consumablesRate!);
...
export const getWelderLoadedRate = ({ proposal }: { proposal: Proposal }): number => {
    const { weldBaseRate, burdenRate, overheadRate, laborProfitRate, fuelRate, consumablesRate, subsistenceRate, rigProfitRate, rigRate } = proposal;
    const ratesSum = sumRates(burdenRate!, overheadRate!, laborProfitRate!, fuelRate!, consumablesRate!, rigProfitRate!);
    return weldBaseRate! + weldBaseRate! * ratesSum + subsistenceRate! + rigRate! + rigRate! * pctToDecimal(rigProfitRate!);
};
...
export const getEquipmentCost = ({
    ...
    return equipmentOwnership === EquipmentOwnership.owned
        ? quantity * time * price
        : quantity * time * price * (1 + sumRates(equipmentProfitRate!, useTaxRate!));
};
...
export const getSubcontractorCost = ({
    ...
    return quantity * (
        ...
        materialCost * (1 + pctToDecimal(subContractorProfitRate!) + pctToDecimal(salesTaxRate!)) +
...
export const getTotalCost = ({
    ...
    const { craftCost, welderCost, materialCost, equipmentCost, subContractorCost, costOnlyCost } = activity;
