...
export function numberToLetters(num: number) {
  ...
  let tempNum = num;
  while (tempNum > 0) {
    const modulo = (tempNum - 1) % 26;
    letters = String.fromCharCode(65 + modulo) + letters;
    tempNum = Math.floor((tempNum - modulo) / 26);
  }
  return letters;
}
...
export const calculateTotals = (activities: Activity[]) => {
  return activities.reduce(
    (acc, activity) => ({
      ...
      subContractorCost:
        acc.subContractorCost + (activity.subContractorCost || 0),
      materialCost:
...
export const calculateWbsTotals = (phases: Phase[]) => {
  return phases.reduce(
    (acc: Accumulator, phase) => ({
...
export function calculateNewSortOrder(
  ...
  const prevSortOrder =
    newIndex > 0
      ? activities[newIndex - 1].sortOrder
      : activities[0].sortOrder - 1;
  const nextSortOrder =
    newIndex < activities.length - 1
      ? activities[newIndex + 1].sortOrder
      : activities[activities.length - 1].sortOrder + 1;
