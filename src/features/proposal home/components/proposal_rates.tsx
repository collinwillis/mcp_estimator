...
function FieldDisplay({ label, value }: { label: string; value: string }) {
  return (
    <Box sx={{ minHeight: 44 }}>
      ...
      <Typography sx={{ fontSize: '0.825rem', fontWeight: 600, color: '#111827', fontVariantNumeric: 'tabular-nums' }}>
        {value}
      </Typography>
    </Box>
  );
...
const ProposalRates: React.FC<ProposalRatesProps> = ({
  ...
  const renderSection = (title: string, fields: { label: string; field: string; prefix?: string; suffix?: string }[]) => (
    <Box sx={{ mb: 3 }}>
      <Typography sx={{ fontSize: '0.675rem', fontWeight: 600, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.05em', mb: 1.5, pb: 0.5, borderBottom: '1px solid #f3f4f6' }}>
        {title}
      </Typography>
          ...
          <Grid item xs={6} sm={4} md={3} key={field}>
            {isEditMode ? (
              <TextField label={label} variant='outlined' size='small' fullWidth name={field}
                ...
                InputProps={{
                  startAdornment: prefix ? <InputAdornment position='start'><Typography sx={{ fontSize: '0.8rem', color: '#6b7280' }}>{prefix}</Typography></InputAdornment> : undefined,
                  endAdornment: suffix ? <InputAdornment position='end'><Typography sx={{ fontSize: '0.8rem', color: '#6b7280' }}>{suffix}</Typography></InputAdornment> : undefined,
                }}
                sx={fieldSx} />
            ) : (
